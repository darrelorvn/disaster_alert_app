@extends('pages.officer.kelola-data.manage-data', ['activeTab' => 'faskes'])
...
<div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 py-5 px-6">
    <form id="filterForm" method="GET" action="{{ route('officer.kelola-data.faskes.index') }}" class="flex flex-col md:flex-row md:items-center gap-3 w-full lg:w-auto">
        ...
        <select name="type" onchange="document.getElementById('filterForm').submit()" class="w-full md:w-auto min-w-[160px] border border-slate-200 rounded-lg bg-white text-slate-600 py-2.5 px-3 text-xs font-bold outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 transition-all cursor-pointer">
            ...
            @foreach($types as $type)
                <option value="{{ $type->value ?? $type }}" {{ request('type') == ($type->value ?? $type) ? 'selected' : '' }}>
        ...
        <select name="status" onchange="document.getElementById('filterForm').submit()" class="w-full md:w-auto min-w-[160px] border border-slate-200 rounded-lg bg-white text-slate-600 py-2.5 px-3 text-xs font-bold outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 transition-all cursor-pointer">
            ...
            <option value="active" {{ request('status') == 'active' ? 'selected' : '' }}>Operasional</option>
        ...
        @if(request()->filled('type') || request()->filled('status'))
            <a href="{{ route('officer.kelola-data.faskes.index') }}" class="text-[10px] font-black text-red-500 hover:text-red-700 uppercase tracking-widest transition-colors">Reset</a>
        @endif
    </form>
    ...
    <div class="flex items-center justify-between lg:justify-end gap-4 w-full lg:w-auto">
        ...
        <a href="{{ route('officer.kelola-data.faskes.create') }}" class="bg-[#FF7F3E] hover:bg-[#e66a2e] text-white py-2.5 px-4 rounded-lg text-[11px] font-black uppercase tracking-wide transition-all shadow-sm flex items-center gap-2">
            ...
            @forelse($places as $place)
                <tr class="hover:bg-slate-50 transition-colors">
                    ...
                    <td class="py-[18px] px-5 border-t border-slate-100 text-[13px] align-middle">
                        <span class="inline-flex items-center justify-center rounded-full py-1 px-2.5 text-[10px] font-black uppercase {{ is_object($place->type) && method_exists($place->type, 'color') ? $place->type->color() : 'bg-slate-100 text-slate-600' }}">
                            {{ is_object($place->type) && method_exists($place->type, 'label') ? $place->type->label() : $place->type }}
                        </span>
                    </td>
                    ...
                    <td class="py-[18px] px-5 border-t border-slate-100 text-[13px] align-middle">
                        <div class="flex items-center gap-2">
                            <a href="{{ route('officer.kelola-data.faskes.edit', $place->id) }}" class="border border-slate-200 rounded bg-white text-slate-700 py-1.5 px-3 text-[10px] font-black cursor-pointer hover:border-orange-400 transition-colors">
                                EDIT
                            </a>
                            
                            <form action="{{ route('officer.kelola-data.faskes.destroy', $place->id) }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data fasilitas ini secara permanen?');">
                                @csrf
                                @method('DELETE')
